...
def evaluate_and_log(experiment_id='99',config=None,):
    ...
    with wandb.init(project="MLOps-Pycon2023_jp", name=f"Eval Model ExecId-{args.IdExecution} ExperimentId-{experiment_id}", job_type="eval-model", config=config) as run:
        data = run.use_artifact('mnist-preprocess:latest')
        data_dir = data.download()
